...
namespace UltraStrafe
{
    internal static class JumpBuffer
    {
        ...
        public static void JumpBufferCheck(NewMovement __instance)
        {
            ...
            if (!GameStateManager.Instance.PlayerInputLocked && jumpBuffer && MonoSingleton<InputManager>.Instance.InputSource.Jump.IsPressed && !jumpCooldownRef(__instance) && (!fallingRef(__instance) || __instance.gc.canJump || wcRef(__instance).CheckForEnemyCols()))
            {
